...
import { useSSRContext, reactive, resolveComponent, mergeProps, withCtx, createVNode, withDirectives, vShow, toDisplayString, createCommentVNode, effectScope, inject, ref, onMounted, unref, resolveDynamicComponent, Transition, openBlock, createBlock, nextTick, withAsyncContext } from "vue";
    ...
    async addPokemons() {
      ...
      const query = `limit=${this.pageLimit}&offset=${this.pageOffset}`;
      {
        ...
        const pokemonsWithData = await Promise.all(
          (await Promise.all(pokemons.map((pokemon) => fetch_(pokemon.url)))).map((response) => response.json()).map((json) => makePokemonJsonLighter(json))
        );
        this.pokemons = this.pokemons.concat(
          pokemonsWithData.filter(
            (pokemonWithData) => pokemonWithData.sprites.front_default != null
          )
        );
      }
    }
...
function makePokemonJsonLighter(json) {
  console.log(json);
  return {
      ...
      _push(ssrRenderComponent(_component_router_link, mergeProps({
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            ])}" w="10px" h="10px" b="rd-2px" relative${_scopeId}></span></div></div></div><div mt="auto"${_scopeId}><div mt="5%" m="x-auto" block text="center #000" case-capital truncate w="85%"${_scopeId}>${ssrInterpolate(props.pokemon.name)}</div><!-- <div 
          m="y-5%"
          block
          text="center 14px #aaa"
          font="semibold">
          # {{ props.pokemon.order }}
        </div> --><div m="y-5%" block text="center 14px #aaa" font="semibold"${_scopeId}> # ${ssrInterpolate(props.pokemon.id)}</div></div></div>`);
          } else {
            return [
              createVNode("div", {
                absolute: "",
                ...
                createVNode("div", { mt: "auto" }, [
                  ...
                  createVNode(
                    ...
                    " # " + toDisplayString(props.pokemon.id),
                    1
                    /* TEXT */
                  )
                ])
...
class InifiniteScroll {
  ...
  run() {
    this.scope.run(() => {
      ...
      useInfiniteScroll(
        ...
        { distance: this.scrollerEl.value.offsetHeight * 2 }
      );
    });
  }
  stop() {
  ...
  onResize() {
    const debouncedRun = debounce(this.run.bind(this), 1e3);
    if (typeof window !== "undefined") {
      window.addEventListener("resize", () => {
        this.stop();
        this.scope = effectScope();
        console.log("new distance", this.scrollerEl.value.offsetHeight * 2);
        debouncedRun();
      });
    }
  }
...
const _sfc_main$5 = {
  ...
  async setup(__props) {
      ...
      if (!nextEvolutionChain) {
        ...
        for (let i = 0; i < nextEvolutionChain.length; i++) {
          toSpecies = {
            ...
            imageSrc: imageSrcBaseUrl + nextEvolutionChain[i].species.url.split("/").slice(-2, -1)[0] + imageSrcExtension,
    ...
    function findNextEvolutionChain(currentEvolutionChain, nameToMatch, selfName) {
      ...
      if (((_a = currentEvolutionChain == null ? void 0 : currentEvolutionChain.species) == null ? void 0 : _a.name) === nameToMatch) {
        if (!selfName) {
          ...
          return currentEvolutionChain.evolves_to.find((chain) => chain.species.name === selfName);
        }
      } else {
        if (!selfName) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (unref(evolutionFlow).length > 0) {
        _push(`<!--[-->`);
        ssrRenderList(unref(evolutionFlow), (evolution) => {
          ...
          ssrRenderList(evolution[1].details, (evolutionDetail) => {
            var _a;
            _push(`<div sm:flex="~ nowrap row" flex="~ col" sm:items="center" b="1px solid #eee rd-3px" mt="2.5%" data-v-810d6c96><span text="center" shrink="0" w="85px" pa="2" pb="0 sm:2" pr="2 sm:0" whitespace="nowrap" data-v-810d6c96>${ssrInterpolate((_a = evolutionDetail == null ? void 0 : evolutionDetail.trigger) == null ? void 0 : _a.name)}</span><div flex="~ col" data-v-810d6c96><!--[-->`);
            ssrRenderList(getEvolutionTriggers(evolutionDetail), (evolutionTrigger) => {
              _push(`<div data-v-810d6c96><div flex items="center" data-v-810d6c96><span grow="1" pa="2" text="12px #aaa justify" leading="16px" data-v-810d6c96>${ssrInterpolate(evolutionTrigger.triggerTypeDescription)}</span><span grow="0" shrink="0" w="100px" pa="2" text="12px right" data-v-810d6c96>${ssrInterpolate(evolutionTrigger.triggerValue)}</span></div></div>`);
            });
            _push(`<!--]--></div><!-- <span
                        pa="2"
        ...
        _push(`<p${ssrRenderAttrs(_attrs)} data-v-810d6c96>-</p>`);
      }
    };
  }
};
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({
        ...
        flex: "",
        items: "center"
      }, _attrs))}><span w="42.5%" text="75%" font="semibold" style="${ssrRenderStyle({
        width: unref(label) === "Abilities" ? "auto" : ""
      })}">${ssrInterpolate(unref(label))}:</span><span text="#000 14px" style="${ssrRenderStyle({
        fontSize: props.valueFontSize || "",
        marginLeft: unref(label) === "Abilities" ? ".5em" : ""
      })}" sm:ml="unset" ml="auto" truncate>${ssrInterpolate(unref(value))}</span></div>`);
    };
  }
};
...
const _sfc_main$2 = {
  ...
  async setup(__props) {
    ...
    const pokemonSpecies$1 = ([__temp, __restore] = withAsyncContext(() => pokemonSpecies.promise), __temp = await __temp, __restore(), __temp);
    const genus = pokemonSpecies$1.genera.find((genera) => genera.language.name === "en").genus;
    const flavorText = (_a = pokemonSpecies$1.flavor_text_entries.find((flavorTextEntry) => flavorTextEntry.language.name === "en")) == null ? void 0 : _a.flavor_text.replaceAll("POKéMON", "Pokémon");
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({
        flex: "",
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      })}" invisible><div w="82.5%" m="x-auto" flex="~ col" sm:max-w="512px" bg="white"><header bg="#63BBBF" pa="5" text="center #fff" relative>${ssrInterpolate(pokemon.name.charAt(0).toUpperCase() + pokemon.name.slice(1))} <!-- <span
                        ml="5"
                        text="#fff"
                        ># {{ pokemon.order }}</span> --><span right="5%" absolute top="50%" translate="y-[-50%]" text="#fff"># ${ssrInterpolate(pokemon.id)}</span></header><section flex pa="5"><!-- image and type --><div w="28.25%" grow="0" flex="col"><!-- imageEl --><div id="imageEl" w="100%" h="0" pb="100%" relative><!-- <div
                                absolute
                                w="100%"
                                h="100%"
                                bg="#eee">
      ...
      ssrRenderList(pokemon.stats, (stat) => {
        _push(`<div flex items="center" class="[&amp;:not(:first-child)]:mt-3.5%"><span grow="0" w="sm:25% 30%" text="sm:16px 12px">${ssrInterpolate(unref(root).pokemonStatsNaming[stat.stat.name])}</span><span class="stats-value" grow="1" b="1px #ddd solid rd-3px" pa="sm:4px 1px" ml="sm:12.5% 10%" bg="#eee" relative overflow="x-hidden"><span relative z="1" p="l-1.5" text="#fff sm:14px 12px">${ssrInterpolate(stat.base_stat)}</span><span absolute left="0" top="50%" style="${ssrRenderStyle({
          width: `${stat.base_stat}%`
        })}" h="105%" translate="y-[-50%]" bg="#CB4D82"></span></span></div>`);
      });
          ...
          ssrRenderSuspense(_push, {
            default: () => {
              _push(ssrRenderComponent(_component_SimpleStat, {
                ...
                "value-font-size": profileEntry.label === "Abilities" ? "10px" : null
              }, null, _parent));
            },
            _: 2
            /* DYNAMIC */

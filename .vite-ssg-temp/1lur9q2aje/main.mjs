...
const _sfc_main$a = {
  ...
  setup(__props) {
    const props = __props;
    const root2 = useRootStore();
    const typeColors = reactive({
      a: "",
      b: ""
    });
    if (props.pokemon.types.length > 1) {
      typeColors.a = root2.pokemonTypes[props.pokemon.types[0].type.name].color;
      typeColors.b = root2.pokemonTypes[props.pokemon.types[1].type.name].color;
    } else {
      typeColors.a = root2.pokemonTypes[props.pokemon.types[0].type.name].color;
      typeColors.b = null;
    }
...
const throttledAddPokemons = throttle(root.addPokemons, 1750, {
  trailing: false
});
...
const _sfc_main$8 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(root2).pokemons, (pokemon) => {
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<!--[--><!-- move type and effect text --><div flex="~ sm:row col" m="x-2"><!-- move type --><span shrink="0" flex self="sm:unset end" items="center" pr="sm:2"><span class="type-palette" w="90px" text="center 10px white" style="${ssrRenderStyle({
        backgroundColor: unref(root2).pokemonTypes[unref(moveDetails).type.name].color
    ...
    const getEvolutionTriggers = (evolutionDetail) => {
      ...
      for (const key in evolutionDetail) {
        ...
        if (evolutionDetail[key] !== null) {
          result.push({
            triggerTypeDescription: root2.evolutionTriggerDescriptionByTriggerType[key],
            triggerValue: (() => {
      ...
      ssrRenderList(unref(pokemonTypesByDamageRelation), (pokemonTypes2, damageRelation) => {
        ...
        ssrRenderList(pokemonTypes2, (pokemonType) => {
          _push(`<span style="${ssrRenderStyle({
            backgroundColor: unref(root2).pokemonTypes[pokemonType.name].color
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const root2 = useRootStore();
    const pokemon = reactive(root2.getPokemonById(+route.params.id)[0]);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(pokemon.types, (type) => {
        _push(`<div class="type-palette sm:text-10px" style="${ssrRenderStyle({
          backgroundColor: unref(root2).pokemonTypes[type.type.name].color
      ...
      ssrRenderList(pokemon.stats, (stat) => {
        _push(`<div flex items="center" class="[&amp;:not(:first-child)]:mt-3.5%"><span grow="0" w="sm:25% 30%" text="sm:16px 12px">${ssrInterpolate(unref(root2).pokemonStatsNaming[stat.stat.name])}</span><span class="stats-value" grow="1" b="1px #ddd solid rd-3px" pa="sm:4px 1px" ml="sm:12.5% 10%" bg="#eee" relative><span relative z="1" p="l-1.5" text="#fff sm:14px 12px">${ssrInterpolate(stat.base_stat)}</span><span absolute left="0" top="0" w="50%" style="${ssrRenderStyle({ width: `${stat.base_stat}%` })}" h="100%" bg="green"></span></span></div>`);
      });
